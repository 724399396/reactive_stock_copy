...
populateStockHistory = (message) ->
  ...
  detailHolder = $("<div>").addClass("detail-holder")
  flipper = $("<div>").addClass("flipper").append(detailHolder).attr("data-content", message.symbol)
...
updateStockChart = (message) ->
  if ($("#" + message.symbol).size > 0)
    ...
    plot.setData([getCharArray(data)])
    # update the yaxes if either the min or max is now out of acceptable range
...
handleFlip = (container) ->
  if (container.hasClass("flipped"))
    ...
    #fetch stock detail and tweet news
    $.ajax
      url: "/sentiment/" + container.children(".flipper").attr("data-content")
      dataType: "json"
      context: container
      success: (data) ->
        detailHolder = $(this).find(".detail-holder")
        detailHolder.empty()
        switch data.lable
          when "pos"
            detailHolder.append($("<h4>").text("The tweets say BUY!"))
            detailHolder.append($("<img>").attr("src", "/assets/images/buy.png"))
          when "neg"
            detailHolder.append($("<h4>").text("The tweets say SELL!"))
            detailHolder.append($("<img>").attr("src", "/assets/images/sell.png"))
          else
            detailsHolder.append($("<h4>").text("The tweets say HOLD!"))
            detailsHolder.append($("<img>").attr("src", "/assets/images/hold.png"))
      error: (jqXHR, textStatus, error) ->
        detailsHolder = $(this).find(".detail-holder")
        detailsHolder.empty()
        detailsHolder.append($("<h2>").text("Error:" + JSON.parse(jqXHR.responseText).error))
      #display loding info
      detailsHolder = $(this).find(".detail-holder")
      detailsHolder.appned("<h4>").text("Determing whether you should buy or sell based on the sentiment of recent tweets...")
